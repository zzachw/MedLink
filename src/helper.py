...
from src.utils import project_path, create_directory, set_seed
...
class Helper:
    ...
    def __init__(self, parse_arguments_fn):
        ...
        args.checkpoint = os.path.join(project_path, f"output/{args.checkpoint}") if args.checkpoint else None
        args.device = self.get_device(args.no_cuda)
        self.args = args

        # set_output_folder
        output_saved_path = os.path.join(project_path, f"output/{args.exp_name}")
        ...
        neptune_logger = None
        if args.official_run and CREDENTIALS_IMPORTED:
